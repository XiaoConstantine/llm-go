...
// returns. For a streaming operation, the borrow ends when the stream is closed.
// Callers must not mutate borrowed storage concurrently. All storage reachable
// from values returned by an implementation belongs to the caller and is not
// mutated by the implementation after delivery.
package llm
